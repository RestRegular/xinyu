...
| **叙事基调** | 情感氛围 | 史诗、严肃、轻松、黑暗、幽默 |
| **叙述视角** | 叙事的人称 | 第二人称（你）、第三人称（他/她）、第一人称（我） |
...
4. **叙述视角**：根据主题选择最合适的视角（见下方指南）
5. **初始剧情提示**：指导 AI 开场时的剧情方向和氛围（可选，留空则 AI 自由发挥）
6. **起始地点**：玩家开始的位置 + 50-100 字的场景描写
7. **初始物品**：4-6 件，符合世界观设定的物品
8. **初始金币**：50-500，符合世界观经济水平
9. **自定义指令**：5-10 条叙事指导，帮助 AI 理解这个世界

#### 叙述视角选择指南

| 视角 | 值 | 适用场景 | 示例 |
|------|------|----------|------|
| 第二人称 | `second_person` | 沉浸式冒险、RPG、恐怖悬疑（默认） | "你推开了古老的石门..." |
| 第三人称 | `third_person` | 小说式叙事、群像剧、史诗战争 | "艾拉握紧了手中的剑..." |
| 第一人称 | `first_person` | 个人视角体验、日记体、侦探推理 | "我注意到桌上的信封有一角被撕开了..." |

> **注意**：`perspective` 字段会影响 GM 提示词、角色 AI 提示词和 UserAgent 提示词中的叙述方式，是全局生效的字段。
...
```jsonc
{
  ...
  "world": {
    ...
    "openingPrompt": "初始剧情提示（可选，50-200字），指导AI开场的剧情方向和氛围"
  },
...
| `world.perspective` | ✅ | string | `second_person` / `third_person` / `first_person` |
| `world.customPrompt` | ✅ | string | 5-10 条指令 |
| `world.openingPrompt` | ⬜ | string | 可选，50-200 字，留空则 AI 自由发挥 |
...
- [ ] JSON 所有必填字段完整
- [ ] `world.perspective` 为三个枚举值之一
- [ ] `world.openingPrompt` 如有填写，内容为 50-200 字的开场方向指导
- [ ] `world.tone` 为最新枚举值（史诗/严肃/轻松/黑暗/幽默）
